...
class PlatformStoreTest extends BaseIntegrationTesting
{
    ...
    public function test_if_cant_see_if_user_is_unauthenticated(): void
    {
        ...
        $this->postJson(route('platforms.store'), $this->getValidPayload())
            ->assertUnauthorized()
    ...
    public function test_if_cant_see_if_hasnt_permissions(): void
    {
        ...
        $this->postJson(route('platforms.store'), $this->getValidPayload())->assertNotFound();
    }

    /**
     * Test if can't create a platform without payload.
     *
     * @return void
     */
    public function test_if_cant_create_a_platform_without_payload(): void
    {
        $this->postJson(route('platforms.store'))->assertUnprocessable();
    }
    ...
    {
        $this->postJson(route('platforms.store'))
            ->assertUnprocessable()
            ->assertInvalid(['name']);
    }
    ...
    /**
     * Test if can't create a duplicated platform.
     *
     * @return void
     */
    public function test_if_cant_create_a_duplicated_platform(): void
    {
        ...
        $this->postJson(route('platforms.store'), $data)
            ->assertUnprocessable()
            ->assertInvalid(['name'])
    ...
    public function test_if_can_log_context_on_platform_creation_failure(): void
    {
        ...
        $logServiceMock->shouldReceive('withContext')
            ...
            ->with(
                Mockery::on(function (string $title) {
                    return $title === 'Failed to create a new platform.';
                }),
        ...
        $platformserviceMock->shouldReceive('create')
            ->once()
            ->andThrow($exception);
    ...
    {
        $this->postJson(route('platforms.store'), $this->getValidPayload())->assertCreated();
    }
    ...
    {
        $this->postJson(route('platforms.store'), $data = $this->getValidPayload())->assertCreated();
    ...
    {
        $this->postJson(route('platforms.store'), $this->getValidPayload())->assertCreated()->assertJsonStructure([
            'data' => [
                'id',
                'name',
                'slug',
    ...
    {
        $this->postJson(route('platforms.store'), $data = $this->getValidPayload())->assertCreated()->assertJson([
            'data' => [
                'name' => $data['name'],
            ],
        ]);
