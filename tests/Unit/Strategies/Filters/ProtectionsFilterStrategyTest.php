...
use App\Strategies\Filters\ProtectionsFilterStrategy;

class ProtectionsFilterStrategyTest extends TestCase
{
    /**
    ...
    public function test_apply_adds_where_has_query(): void
    {
        ...
        $builderMock->shouldReceive('whereHas')
            ...
            ->with('crack', Mockery::on(function (callable $closure) use ($slug) {
                ...
                $nestedBuilderMock->shouldReceive('whereHas')
                    ->once()
                    ->with('protection', Mockery::on(function (callable $innerClosure) use ($slug) {
        ...
        $strategy = new ProtectionsFilterStrategy();
